...
import pytest
from stko import MacroModelForceField, MacroModelMD
...
def macromodel_forcefield(tmpdir, macromodel_path):
    return MacroModelForceField(
        force_field=16, output_dir=tmpdir, macromodel_path=macromodel_path
    )
...
class TestMacroModelFF:
    def test_generate_com(
        self, macromodel_forcefield, tmp_path, benzene_build
    ):
        # Write the ``.com`` file.
        run_name = "test_com"
