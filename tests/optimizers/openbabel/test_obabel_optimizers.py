...
def test_open_babel(case_molecule):
    if openbabel is None:
        with pytest.raises(stko.WrapperNotInstalledException):
            energy = stko.OpenBabelEnergy("uff").get_energy(
        ...
        opt_energy = stko.OpenBabelEnergy("uff").get_energy(opt_res)
        assert np.isclose(energy, case_molecule.unoptimised_energy, atol=1e-3)
